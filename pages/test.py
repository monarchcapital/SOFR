...
if st.button("Run Whole-Instrument Anchor Shock"):
    try:
        ...
        delta_spreads = (L_sp @ delta_PC) * std3.values
        ...
        pivot = contract_labels[len(contract_labels) // 2]
        ...
        for c, _ in legs:
            ax.scatter(c, outr_mkt[c], s=160, color='red', zorder=5)
...
def _kalman_filter_1d(observed, phi, q, r):
    n = len(observed)
    x_hat = np.zeros(n)
    P = np.zeros(n)
    x_hat[0] = observed[0]
...
if use_kalman:
    for i in range(pc_count):
        ...
        q = 0.01 * var   # process noise
        r = 1.00 * var   # measurement noise

        kalman_scores[pc_name] = _kalman_filter_1d(
            raw_series, phi=phi, q=q, r=r
        )
    ...
    st.dataframe(pd.DataFrame(phi_rows), use_container_width=True)
...
# ------------------------------------------------------------
# 12.0 Expression quality of selected instrument
# ------------------------------------------------------------
...
def compute_expression_quality(instr, factor_df, Sigma, mispricing):
    ...
    return {
        ...
        "Expression Quality Score": expr_quality
    }
...
def find_alternatives(T, universe_df, factor_df, Sigma, mispricing, top_n=5):
    ...
    local = universe_df[universe_df["Instrument"].str.contains(maturity_tag)]

    rows = []
    for C in local["Instrument"]:
...
residual_diag = pd.DataFrame({
    ...
    "Value": [
        combo["Target Factor"],
        combo["Residuals"].get("Level (Whole Curve Shift)", np.nan),
        combo["Residuals"].get("Slope (Steepening/Flattening)", np.nan),
        combo["Residuals"].get("Curvature (Fly Risk)", np.nan),
